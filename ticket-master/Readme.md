...
- **How can we make it even more faster?** What about popular queries? *like every body is searching for Arijit Singh's concert*
   ...
   ![cdn](image-7.png)
      It is Superfast but become less useful when you have more permutations of your search queries like search term, lat, long, type or date, it will be inefficient.
...
The choke point would be introducing virtual waiting queue, this virtual waiting queue can only be enabled for very popular events (let say admin can enable them)
...
We can have some event driven logic like we let the 1st 100 people in, once 100 seats are booked we let the next 100 people in and so on. So they are pulled off of the queue and those uses are notified (on the same sse connection created prior), they are let in and can book ticket.
